...
RUSTFLAGS="-C target-feature=+neon -C opt-level=3 -C strip=symbols" cargo +nightly build --target aarch64-linux-android --features nightly_i8mm --release
...
#
RUSTFLAGS="-C target-feature=+sse4.1 -C opt-level=3 -C strip=symbols" cargo +nightly build --target x86_64-linux-android --release
...
#
RUSTFLAGS="-C strip=symbols" cargo +nightly build --target armv7-linux-androideabi --release
...
RUSTFLAGS="-C strip=symbols" cargo +nightly build --target i686-linux-android --release
